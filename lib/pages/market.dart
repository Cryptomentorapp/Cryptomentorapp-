...
class _MarketPageState extends State<MarketPage>{
  ...
  @override Widget build(BuildContext context){
    ...
    return RefreshIndicator(onRefresh: fetch, child: ListView.separated(
      ...
      itemBuilder: (_,i){ final Map<String,dynamic> c = Map<String,dynamic>.from(items[i] as Map);
        final String img=(c['image']??'').toString();
        final String name=(c['name']??'').toString();
        final String sym=(c['symbol']??'').toString().toUpperCase();
        final String price=(c['current_price'] is num)? (c['current_price'] as num).toStringAsFixed(4) : (c['current_price']?.toString() ?? '-');
        final String ch24=(c['price_change_percentage_24h'] is num)? (c['price_change_percentage_24h'] as num).toStringAsFixed(2)+'%' : '-';
