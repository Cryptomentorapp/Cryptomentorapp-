...
class _WhaleTrackerPageState extends State<WhaleTrackerPage> {
  ...
  @override Widget build(BuildContext context){
    ...
    final list=q.isEmpty?app.whalesTop50:app.whalesTop50.where((e)=> (e['address'] as String).toLowerCase().contains(q.toLowerCase())).toList();
    return Column(children:[
      Padding(padding: const EdgeInsets.all(12), child: TextField(decoration: const InputDecoration(prefixIcon: Icon(Icons.search), labelText:'Filter address'), onChanged: (v)=>setState(()=>q=v))),
      Expanded(child: ListView.separated(padding: const EdgeInsets.all(12), itemBuilder: (_,i){
        final t=list[i];
        return Card(child: ListTile(
          leading: const Icon(Icons.account_balance_wallet_outlined),
          title: Text('${t['address']}'),
          subtitle: Text('Chain: ${t['chain']} · Watchlist'),
