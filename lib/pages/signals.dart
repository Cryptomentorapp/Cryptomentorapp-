  ...
  @override Widget build(BuildContext context){
    ...
    return Column(children:[
      ...
      Expanded(child: ListView.separated(padding: const EdgeInsets.all(12), itemBuilder: (_,i){
        ...
        return Card(child: ListTile(title: Text('${s['pair']} · ${s['time']}'), subtitle: Text('Entry ${s['entry']} · SL ${s['sl']} · TP ${s['tp']}'),
          trailing: Chip(label: Text(s['status']!), backgroundColor: color.withOpacity(.15), labelStyle: TextStyle(color: color)),));}, separatorBuilder: (_,__)=>const SizedBox(height:8), itemCount: filtered.length))
    ]);
  }
}