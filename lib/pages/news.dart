  ...
  Future<void> loadNews() async {
    ...
    try{
      ...
      if(cp.statusCode==200){ final doc=xml.XmlDocument.parse(cp.body);
        fast = doc.findAllElements('item').take(20).map((n)=>_NewsItem(
          title: n.getElement('title')?.text ?? '', url: n.getElement('link')?.text ?? '', source: 'CryptoPanic',
          pubDate: n.getElement('pubDate')?.text ?? ''
        )).toList();
      }
      ...
      if(ct.statusCode==200){ final doc=xml.XmlDocument.parse(ct.body);
        deep = doc.findAllElements('item').take(20).map((n)=>_NewsItem(
          title: n.getElement('title')?.text ?? '', url: n.getElement('link')?.text ?? '', source: 'CoinTelegraph',
          pubDate: n.getElement('pubDate')?.text ?? ''
        )).toList();
      }
