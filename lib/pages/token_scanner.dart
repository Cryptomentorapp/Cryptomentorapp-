...
class _TokenScannerPageState extends State<TokenScannerPage> with TickerProviderStateMixin {
  ...
  Future<void> loadHotlist() async {
    try{ final res=await http.get(Uri.parse('https://api.coingecko.com/api/v3/search/trending'));
      if(res.statusCode==200){ final data=jsonDecode(res.body) as Map<String,dynamic>; final coins=(data['coins'] as List?)??[];
        hotlist = coins.map((e){ final item=(e as Map)['item']??{}; return _HotToken(name:item['name']??'', symbol:item['symbol']??'', id:item['id']??'', rank:item['market_cap_rank']); }).toList().cast<_HotToken>(); }
      setState((){});
    } catch(_){}
  }
  Future<void> search() async {
    setState(()=>loading=true);
    try{ final res=await http.get(Uri.parse('https://api.coingecko.com/api/v3/search?query='+Uri.encodeComponent(queryCtrl.text)));
      if(res.statusCode==200){ final data=jsonDecode(res.body) as Map<String,dynamic>; results=(data['coins'] as List?)??[]; error=null; } else { error='HTTP '+res.statusCode.toString(); }
    } catch(e){ error=e.toString(); }
    setState(()=>loading=false);
  }
  Future<void> scanById(String id) async {
    setState(()=>loading=true);
    try{ final res=await http.get(Uri.parse('https://api.coingecko.com/api/v3/coins/'+id+'?localization=false&tickers=false&market_data=true&community_data=false&developer_data=false&sparkline=false'));
      if(res.statusCode==200){ detail=jsonDecode(res.body) as Map<String,dynamic>; error=null; tab.animateTo(0); } else { error='HTTP '+res.statusCode.toString(); } }
    catch(e){ error=e.toString(); }
    setState(()=>loading=false);
  }
  ...
  Widget _buildScan(){
    return Column(children:[
      ...
      if(detail==null) Expanded(child: ListView.separated(
        padding: const EdgeInsets.all(12), itemCount: results.length,
        itemBuilder: (_,i){ final r=Map<String,dynamic>.from(results[i] as Map);
          return Card(child: ListTile(
            leading: CircleAvatar(backgroundImage: NetworkImage((r['thumb']??'').toString())),
            title: Text((r['name']??'').toString()+' ('+(r['symbol']??'').toString().toUpperCase()+')'),
            subtitle: Text('Rank: '+ ((r['market_cap_rank']??'-')).toString() ),
            trailing: const Icon(Icons.chevron_right),
            onTap: ()=>scanById((r['id']??'').toString()),
          ));
    ...
    return RefreshIndicator(onRefresh: () async { await loadHotlist(); }, child: ListView.separated(
      padding: const EdgeInsets.all(12), itemCount: hotlist.length, itemBuilder: (_,i){
        final h=hotlist[i]; return Card(child: ListTile(
          title: Text(h.name+' ('+h.symbol.toUpperCase()+')'), subtitle: Text('Trend · Rank #'+(h.rank?.toString() ?? '-')),
...
class _DetailView extends StatelessWidget {
  ...
  @override Widget build(BuildContext context){
    ...
    return ListView(padding: const EdgeInsets.all(16), children:[
      ...
      Text('Giá hiện tại: '+_usd(price)),
      Text('Market cap: '+_usd(mcap)),
      Text('Volume 24h: '+_usd(vol)),
  ...
  static String _riskLabel(Map<String,dynamic> d){
    final mcap=d['market_data']?['market_cap']?['usd']??0; final vol=d['market_data']?['total_volume']?['usd']??0; int score=0;
    if(mcap is num && mcap<5000000) score+=2; if(vol is num && vol<200000) score+=1;
    if(score>=3) return 'HIGH RISK'; if(score>=2) return 'MEDIUM'; return 'SAFE';
  }
  static String _riskExplain(Map<String,dynamic> d){
    ...
    if(mcap is num and mcap<5000000) r.add('Market cap nhỏ (<\\$5M)'); if(vol is num and vol<200000) r.add('Volume 24h thấp (<\\$200k)');
